...
class RouteReportHTML(models.Model):
    _name = 'report.sanbe_hr_tms.report_ot_route_html'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for obj in lines:
            ...
            line_data = {
                ...
                'route': route_name,
                'department': obj.department_id.complete_name or 'Tidak Diketahui',
            ...
            if route_name not in grouped_lines:
                grouped_lines[route_name] = []

            grouped_lines[route_name].append(line_data)

        report_lines = []
        for route_name, lines_list in grouped_lines.items():
            for idx, line in enumerate(lines_list, 1):
                line['no'] = idx  # nomor urut per-route
            report_lines.append({
                'route': route_name,
                'lines': lines_list,
            })
        ...
        area = self.env.user.area.name or 'Tidak Diketahui'
        _logger.info(f"Processed {len(report_lines)} routes for area {area}")
