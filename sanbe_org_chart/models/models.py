...
class OrgChartDepartment(models.Model):
	...
	def get_employee_data(self):
		...
		myterritory = self.env['res.territory'].sudo().search([('company_id','=',company.id)])
		cnt =-2
		datadua = []

		dataemp =[]
		for alltery in myterritory:
			databranch = []
			for allbranch in alltery.branch_id:
				mybranch = self.env['res.branch'].sudo().search([('name','=', allbranch.name)], limit=1)
				employee = self.env['hr.department'].search([
					('parent_id', '=', False),
					('branch_id', '=', mybranch.id),
				])
				for allemp in employee:
					dataemp.append(self.get_children(allemp, 'middle-level'))
				databranch.append({
					'id': allbranch.id,
					'name': mybranch.name,
					'title': company.country_id.name,
				...
				dataemp =[]
			datadua.append({
				'id': alltery.id, 'name': alltery.name, 'title': "", 'office': "",
				'children': databranch
			})

		data['children'] = datadua
		# for employee in employees:
		# 	data['C'].append(self.get_children(employee, 'middle-level'))
		return {'values': data}
	...
	def get_children(self, emp, style=False):
		data = []
		company = self.env.user.company_id
		emp_data = {'id':emp.id,'name': emp.name, 'title': "", 'office': ""}
		...
		for child in childrens:
			...
			if not sub_child:
				data.append({'id':child.id ,'name': child.name, 'title': "", 'className': next_style, 'office': ""})
			else:
	...
	def _get_image(self, emp, style):
		...
		image_path = "/sanbe_org_chart/static/src/img/default_image.png"
		return '<img class="avatar %s" src=%s />' % (style, image_path)
	...
	def get_emp_form_id(self):
		org_chart_config = self.env.ref('sanbe_org_chart.slife_org_chart_config_data')
		return {
			'form_id': self.env.ref('sanbe_org_chart.hr_department_branch_form').id,
...
class SlifeEmployee(models.TransientModel):
	...
	def _default_image(self):
		image_path = get_module_resource('sanbe_org_chart', 'static/src/img', 'default_image.png')
		return base64.b64encode(open(image_path, 'rb').read())

	parent_id = fields.Many2one('hr.department', string='Employee Parent', default=_default_parent_id, readonly=True)
	...
	def action_to_save_parent(self):
		for record in self:
			...
			childs = record.env['hr.deoartment'].search([('parent_id','=',record.parent_id.id)])
			for child in childs:
	...
	def _change_information(self, child, department_id, parent_id=False):
		for record in self:
			...
			child.write(vals)
			childrens = record.env['hr.department'].search([('parent_id','=',child.id)])
