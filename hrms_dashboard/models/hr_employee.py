...
class HrEmployee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        """
        cr.execute(qemp)
        new_employee_count = cr.fetchall()
        qempres = """ 
            select count(id)
            from hr_employee
        ...
        """
        cr.execute(qempres)
        emp_resign_count = cr.fetchall()
        qemppermanent = """
            select count(id)
            from hr_employee 
            where job_status ='permanent';
        """
        cr.execute(qemppermanent)
        emp_permanent_count = cr.fetchall()
        ...
        if employee:
            ...
            if employee[0]['birthday']:
                ...
                age = False
            mybak = self.env['hr.employee'].sudo().search(
            [('user_id', '=', uid)], limit=1)
            myarea = mybak.area.name
            mybu = mybak.branch_id.name
            ...
            if employee:
                data = {
                    ...
                    'new_employee_resign_count': emp_resign_count,
                    'emp_permanent_count': emp_permanent_count,
                }
    ...
    def get_dept_employee(self):
        cr = self._cr
        cr.execute("""select kpi_kategory, count(*)
        from hr_employee
        group by kpi_kategory""")
        dat = cr.fetchall()
        data = []
        # for alldata in dat:
